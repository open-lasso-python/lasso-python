...
def _mark_dead_eles(node_indexes: np.ndarray,
                    alive_shells: np.ndarray,
                    node_coordinates: np.ndarray) -> np.ndarray:
    '''
    ...
    node_coordinate_mask: np.ndarray
        Array containing indizes of alive shells. Use node_coordinates[node_coordinate_mask] to get all nodes alive
...
def _extract_shell_parts(part_list: Sequence[int], d3plot: D3plot) -> Union[Tuple[np.ndarray, np.ndarray], str]:
    '''
    ...
    if ArrayType.element_shell_is_alive in d3plot.arrays:
        dead_shell_mask = _mark_dead_eles(shell_node_indexes,
                                          d3plot.arrays[ArrayType.element_shell_is_alive],
                                          node_coordinates)
        alive_mask[dead_shell_mask] = False
    if ArrayType.element_beam_is_alive in d3plot.arrays:
        dead_beam_mask = _mark_dead_eles(beam_node_indexes,
                                         d3plot.arrays[ArrayType.element_beam_is_alive],
                                         node_coordinates)
        alive_mask[dead_beam_mask] = False
    if ArrayType.element_solid_is_alive in d3plot.arrays:
        dead_solid_mask = _mark_dead_eles(solid_node_indexes,
                                          d3plot.arrays[ArrayType.element_solid_is_alive],
                                          node_coordinates)
        alive_mask[dead_solid_mask] = False
    if ArrayType.element_tshell_is_alive in d3plot.arrays:
        dead_tshell_mask = _mark_dead_eles(tshell_node_indexes,
                                           d3plot.arrays[ArrayType.element_tshell_is_alive],
                                           node_coordinates)
    ...
    if(len(part_list) > 0):
        ...
        def mask_parts(part_list2: List[int], element_part_index: np.ndarray,
                       element_node_index: np.ndarray) -> np.ndarray:
            ...
            unique_element_node_indexes = np.unique(
                element_node_index[element_part_filter]
            )

            return unique_element_node_indexes

        # shells:
        unique_shell_node_indexes = mask_parts(
            part_list, shell_part_indexes, shell_node_indexes)

        # beams
        unique_beam_node_indexes = mask_parts(
            part_list, beam_part_indexes, beam_node_indexes)

        # solids:
        unique_solide_node_indexes = mask_parts(
            part_list, solid_part_indexes, solid_node_indexes)

        # tshells
        unique_tshell_node_indexes = mask_parts(
            part_list, tshell_part_indexes, tshell_node_indexes)
...
def create_reference_subsample(load_path: str,
                               parts: Sequence[int],
                               nr_samples=2000) -> Union[Tuple[np.ndarray, float, float], str]:
    '''
    ...
    t_null = time.time()
    try:
        plot = D3plot(load_path, state_array_filter=[ArrayType.node_displacement,
...
def remap_random_subsample(load_path: str,
                           parts: list,
                           reference_subsample: np.ndarray) -> Union[Tuple[np.ndarray, float, float], str]:
    '''
   ...
   '''
    ...
    quarantine_zone = NearestNeighbors(
        n_neighbors=1, n_jobs=4).fit(coordinates)
