...
from lasso.dimred.svd.KeywordTypes import DetectorType, ClusterType
...
class TestClustering(TestCase):
    ...
    def test_group_betas(self):
        ...
        beta_clusters, name_clusters = group_betas(fake_names, fake_betas,
        ...
        self.assertEqual(len(beta_clusters),
                         expected_clusters + expected_outliers)
        ...
        for c, cluster in enumerate(name_clusters):
            for e, entry in enumerate(cluster):
                ...
                self.assertTrue(
                    (fake_betas[index] - beta_clusters[c][e]).max() == 0)
