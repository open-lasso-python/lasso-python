...
from lasso.dimred.svd.subsampling_methods import (create_reference_subsample,
                                                  remap_random_subsample)
from lasso.dimred.testPlotCreator import create_2_fake_plots
...
class TestSubsampling(TestCase):
    ...
    def test_create_reference_sample(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            result = create_reference_subsample(
                load_path, parts=[], nr_samples=n_nodes)
            ...
            # should return string error message if desired samplesize is greater than avaiable nodes
            n_nodes = 5500
            result = create_reference_subsample(
                load_path, parts=[], nr_samples=n_nodes)
            ...
            n_nodes = 200
            result = create_reference_subsample(
                load_path, parts=[1], nr_samples=n_nodes)
    ...
    def test_remap_random_subsample(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            ref_result = create_reference_subsample(
                ref_path, parts=[], nr_samples=n_nodes)
            ...
            # sub_result should be Tuple containing subsample, total process time, and plot load time
            ...
            # this is only true for for the dimredTestPlots, this might not be the case with real plots
            # we check if the difference is 0
            self.assertTrue((ref_sample - subsample[0]).max() == 0)

            # should return string error message for nonexistant parts:
            err_msg = remap_random_subsample(
                sample_path, parts=[1], reference_subsample=ref_sample)
