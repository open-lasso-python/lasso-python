...
def createFakeD3Plots(path: str, element_shell_node_indexes: np.ndarray, bend_multiplicator: float,
                      n_nodes_x: int = 500, n_nodes_y: int = 10, n_timesteps: int = 5):
    '''
    ...
    # z_bend_mat = np.stack([np.array([1+math.sin(x*2*math.pi/(bend_end - bend_start)) for x in range(bend_end - bend_start)])
    ...
    # we could create an artificial array element_shell_is_alive to test the correct part extraction process
...
def plotUtilFunc(xyz_array: np.array):
    trace = go.Scatter3d(
        x=xyz_array[-1, :, 0],
        y=xyz_array[-1, :, 1],
        z=xyz_array[-1, :, 2],
...
def create_element_shell_node_indexes(n_nodes_x: int = 500, n_nodes_y: int = 10) -> np.ndarray:
    ...
    for i in range(n_nodes_y-1):
        new_shell_node_indexes[(
            n_nodes_x-1) * i: (n_nodes_x-1)+((n_nodes_x-1)*i)] += (mod + i*n_nodes_x).T
...
def create_2_fake_plots(folder: str, n_nodes_x: int, n_nodes_y: int, n_timesteps=5):
    ...
    createFakeD3Plots(
        path=os.path.join(folder, plot_name.format(i="00")), element_shell_node_indexes=element_shell_node_indexes,
        bend_multiplicator=5*(1+randy_random.random()), n_nodes_x=n_nodes_x, n_nodes_y=n_nodes_y, n_timesteps=n_timesteps)

    createFakeD3Plots(
        path=os.path.join(folder, plot_name.format(i="01")), element_shell_node_indexes=element_shell_node_indexes,
        bend_multiplicator=5*(1+randy_random.random()), n_nodes_x=n_nodes_x, n_nodes_y=n_nodes_y, n_timesteps=n_timesteps)
...
def create_50_fake_plots(folder: str, n_nodes_x: int, n_nodes_y: int, n_timesteps=5):
    ...
    for i in range(25):
        ...
        if(i < 10):
            nr = "0" + str(i)
        createFakeD3Plots(path=os.path.join(folder, plot_name.format(
            i=nr)), element_shell_node_indexes=element_shell_node_indexes, bend_multiplicator=5*(1+randy_random.random()),
            n_nodes_x=n_nodes_x, n_nodes_y=n_nodes_y, n_timesteps=n_timesteps)

    # 25 plots bending down
    for i in range(25):
        createFakeD3Plots(path=os.path.join(folder, plot_name.format(
            i=i+25)), element_shell_node_indexes=element_shell_node_indexes, bend_multiplicator=-5*(1+randy_random.random()),
            n_nodes_x=n_nodes_x, n_nodes_y=n_nodes_y, n_timesteps=n_timesteps)
