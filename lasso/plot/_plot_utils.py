...
<html lang="en">
  ...
  <body>
    ...
    {{
      ...
      plot.innerHTML = '';
      Plotly.plot(plot, data, layout, {{ showLink: false, modeBarButtonsToRemove: ['sendDataToCloud'] }});
...
def plotly_3d(x, y, z, fringe=None, fringe_names=None, filepath=None, title=""):
    ...
    if fringe.ndim > 1:
        ...
        button_menu_str = _PLOTLY_UPDATEMENU.format(
            _buttons=buttons_str
        )
    ...
    if filepath:
        with open(filepath, "w") as plot_file:
            plot_file.write(_PLOTLY_TEMPLATE.format(
                TITLE=title,
                X_PTS=array2str(x),
                Y_PTS=array2str(y),
                Z_PTS=array2str(z),
