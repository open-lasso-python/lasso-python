...
SCRIPT_STRING = """
    ...
    function showInputField(div, aClicked){
      ...
      document.getElementById(aClicked).style.background = "var(--menu-option-active)"
    }
    ...
    function hideInputField(div, aClicked){
      ...
      document.getElementById(aClicked).style.background = "var(--menu-option-background)"
    }
    ...
    function hideAll(){
      ...
      Array.from(document.getElementById("mySidenav").children).forEach(node => {
        if(node.childElementCount > 0){
          node.children[0].style.background = "var(--menu-option-background)"
        }
      })
    }
